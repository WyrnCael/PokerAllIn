...
public class TableGUI extends JFrame implements ActionListener {
	...
	public TableGUI() {
		...
		listRanking = new SklanskyChubukov();
		panelPrincipal = this;
		initGUI();
		...
		calculateRange.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!selectedList.isEmpty()) {
					// Limpio el string para que no se concatene con lo anterior
					// en cada llamada
					finalRange = "";
					ordenarLista();
					calcularRango();
	...
	public void updateButton(String buttonName, String mode, int increDecre) throws IOException {
		...
		while (!finish) {
			...
			switch (mode) {
			case "Diagonal":
				// si es diagonal, actualizar las dos letras del boton, AA -> KK
				// o KK -> AA
				tmp = parseNum(parseChar(tmp.charAt(0)) + increDecre) + parseNum(parseChar(tmp.charAt(1)) + increDecre);
				break;
			case "VertHor":
				// si es horizontal o vertical, solo actualiza la segunda, AK ->
				// AQ o Q5 -> Q6
	...
	public void actionPerformed(ActionEvent e) {
		JButton boton = (JButton) e.getSource();
		String texto = (boton).getText();
		// El boton ya esta pulsado, se vuelve al color original y se eliminar
		// de la lista
	...
	private void ordenarLista() {
		...
		this.selectedList.sort(new Comparator<JButton>() {
			...
			public int compare(JButton o1, JButton o2) {
				// Como no hay duplicados solo hay que devolver si el boton 1 es
				// mayor que el boton 2
	...
	/**
	 * Recibe un boton y devuelve el valor asociado para poder ordenarlo en la
	 * lista, el valor va de menor a mayor (offsuited/suited/pair)
	 * 
	 * @param name
	 *            La informacion del boton, para poder extraer el valor de la
	 *            mano pulsada
	...
	private int getButtonValue(String name) {
		...
		case 2:
			value = parseChar(name.charAt(0)) + parseChar(name.charAt(1)) + 340;
			break;
		case 3:
			value = (12 * parseChar(name.charAt(0))) + parseChar(name.charAt(1));
			// como el total de combinaciones es 169, separo entre sweater y off
			// sweater
			value += name.charAt(2) == 's' ? 170 : 0;
			break;
		default:
	...
	private void refreshPercentaje() {
		...
		double per = listRanking.getPercent(nSuited, nPairs, nOffSuited);
	...
	private void drawCustomPercent() {
		...
		List<String> ranking = listRanking.getList(percent);
